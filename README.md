...
- S3 업로드와 DB 저장 중 어느 단계에서 실패하더라도 정합성을 어떻게 회복할 수 있을까?
- ZIP Bomb과 Zip Slip을 각각 어떤 방식으로 방어했는가?
- 지연 삭제를 선택했을 때 얻는 복구 가능성과 추가되는 운영 복잡도는 무엇인가?
- 트래픽이 증가한다면 파일 정리 배치와 Presigned URL 권한 모델을 어떻게 확장할 것인가?

---

이 README는 팀 전체 결과를 소개하되, 개인 기여는 Git 작성자 이력과 현재 구현에서 확인되는 파일 도메인 작업을 중심으로 기술했습니다.
